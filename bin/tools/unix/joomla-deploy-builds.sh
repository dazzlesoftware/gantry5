...
shopt -s nullglob
package_archives=("$dist"/joomla-pkg_genesis_"$build_suffix".zip); theme_archives=("$dist"/joomla-tpl_*_"$build_suffix".zip)
...
theme_slugs=()
for archive in "${theme_archives[@]}"; do base="${archive##*/}"; base="${base%.zip}"; slug="${base#joomla-tpl_}"; slug="${slug%_$build_suffix}"; [[ -n "$slug" && "$slug" != "$base" ]] || die "Unable to determine theme name from archive: $archive"; theme_slugs+=("$slug"); done
...
for slug in "${theme_slugs[@]}"; do [[ -d "$templates/tpl_$slug" ]] || die "Expected installed template directory for '$slug': $templates/tpl_$slug"; done
