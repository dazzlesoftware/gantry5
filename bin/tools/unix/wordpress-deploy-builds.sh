...
theme_archives=("$dist"/wordpress-tpl_*_"$build_suffix".zip)
...
for theme in "$themes"/wp_*; do [[ -e "$theme" ]] || continue; path_is_within "$theme" "$themes" || die "Refusing to remove a theme outside $themes"; rm -rf -- "$theme"; done
