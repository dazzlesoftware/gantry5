...
class Filepicker extends JsonController
{
    ...
    public function index()
    {
        ...
        if (!$subfolder) {
            $response['html'] = $this->render(
                '@genesis-admin/ajax/filepicker.html.twig', [
                    'active'    => $active,
                    'base'      => $this->base,
                    'bookmarks' => $bookmarks,
                    'folders'   => $folders,
            ...
            if ($current && $count) {
                $response['subfolder'] = $this->render(
                    '@genesis-admin/ajax/filepicker/subfolders.html.twig',
                    ['folder' => $current]
                );
            } else {
            ...
            $response['files'] = $this->render(
                '@genesis-admin/ajax/filepicker/files.html.twig',
    ...
    protected function doDownload($path, $download)
    {
        ...
        if (!$download && $isImage) {
            ...
            header('Content-Disposition: inline; filename="' . Genesis::basename($targetPath) . '"');
        } else {
            // Force file download.
            header('Expires: 0');
            header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
            ...
            header('Content-Disposition: attachment; filename="' . Genesis::basename($targetPath) . '"');
        }
    ...
    public function upload()
    {
        ...
        // Check extension
        $uploadedName = isset($uploadedFile['name']) && is_string($uploadedFile['name']) ? Genesis::basename($uploadedFile['name']) : '';
