...
class EventListener implements EventSubscriberInterface
{
    ...
    public function onGlobalSave(Event $event): void
    {
        ...
        \update_option('genesis_plugin', $option);
    }
    ...
    public function onMenusSave(Event $event): void
    {
        ...
        foreach ($menu_items as $wpItem) {
            ...
            if ($wpItem->type === 'custom' && !isset($items[$path]) && strpos($wpItem->attr_title, 'genesis-particle-') === 0) {
                $db_id = $wpItem->db_id;

                $debug['delete_' . $db_id] = ['id' => $db_id];

                if (Menu::WRITE_DB) {
                    \delete_post_meta($db_id, '_menu_item_genesis');
        ...
        foreach ($items as $key => $item) {
            ...
            if (!empty($item['id']) && isset($menu_items[$item['id']])) {
                ...
                if (Menu::WRITE_DB) {
                    ...
                    if (Menu::WRITE_META) {
                        \update_post_meta($db_id, '_menu_item_genesis', \wp_slash(json_encode($meta)));
                    } else {
                        \delete_post_meta($db_id, '_menu_item_genesis');
                    }
                }
                ...
                $args = [
                    ...
                    'menu-item-attr-title' => 'genesis-particle-' . $particle,
                    'menu-item-target' => $item['target'] !== '_self' ? $item['target'] : '',
                    'menu-item-classes' => \wp_slash(trim($item['class'])),
                    'menu-item-xfn' => ''
                ];
