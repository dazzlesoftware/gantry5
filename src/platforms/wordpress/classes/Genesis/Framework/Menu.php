...
class Menu extends AbstractMenu
{
    ...
    public function getMenus(array $args = []): array
    {
        if($this->menus === null) {
            ...
            $menus = \wp_get_nav_menus(\apply_filters('genesis_menu_get_menus_args', \wp_parse_args($args, $defaults)));
    ...
    protected function createMenuItemFromPost(\WP_Post $post): Item
    {
        ...
        // Add properties from post meta `_menu_item_genesis`.
        ...
        if ('custom' === $properties['type']) {
            if (strpos($properties['link_title'], 'genesis-particle-') === 0) {
