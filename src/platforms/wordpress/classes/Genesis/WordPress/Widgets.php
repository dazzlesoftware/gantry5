...
abstract class Widgets
{
    ...
    public static function displayPosition($key, array $params = [])
    {
        ...
        \add_filter('dynamic_sidebar_params', ['Genesis\Wordpress\Widgets', 'sidebarChromeFilter'], -1000);
        ...
        if (!empty($params['prepare_layout'])) {
            ...
            foreach ($widgets as $id) {
                ...
                if (empty($wp_registered_widgets[$id]['genesis'])
        ...
        \remove_filter('dynamic_sidebar_params', ['Genesis\Wordpress\Widgets', 'sidebarChromeFilter'], -1000);
                ...
                foreach (\get_option("widget_{$base}") as $id => $instance) {
                    if (is_numeric($id)) {
                        ...
                        if (!$isParticle) {
                            ...
                            if (isset($instance['genesis_classes'])) {
                                $wordpress['genesis_classes'] = $instance['genesis_classes'];
                                unset($instance['genesis_classes']);
                            }
                            $particle = $instance;
                        }
    ...
    public static function widgetCustomClassesForm($widget, $return, $instance)
    {
        ...
        ?>
        <p>
            <label for="<?php echo $widget->get_field_id('genesis_classes'); ?>"><?php \_e('Custom class(es):', 'genesis'); ?></label>
            <input type="text" class="widefat" id="<?php echo $widget->get_field_id('genesis_classes'); ?>"
                   name="<?php echo $widget->get_field_name('genesis_classes'); ?>" value="<?php echo \esc_attr($instance['genesis_classes']); ?>"/>
            <br />
            <small><?php \_e('Multiple class names must be separated by white space characters.', 'genesis'); ?></small>
        </p>
        <?php
    ...
    public static function widgetCustomClassesSidebarParams($params)
    {
        ...
        if (!empty($instance['genesis_classes'])) {
            $params[0]['before_widget'] = preg_replace('/class="/', sprintf('class="%s ', $instance['genesis_classes']), $params[0]['before_widget'], 1);
        }
