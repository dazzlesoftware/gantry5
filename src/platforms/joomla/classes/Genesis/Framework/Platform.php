...
use Genesis\Joomla\Category\CategoryFinder;
use Genesis\Joomla\Content\Content;
use Genesis\Joomla\Content\ContentFinder;
...
class Platform extends BasePlatform
{
    ...
    public function init(): static
    {
        ...
        if ($theme && is_dir(JPATH_ROOT . "/media/genesis/themes/{$theme}/media-shared")) {
            $custom = JPATH_ROOT . "/media/genesis/themes/{$theme}/custom";
            if (!is_dir($custom)) {
                // First run -- copy configuration into a single location.
                $shared = JPATH_ROOT . "/media/genesis/themes/{$theme}/template-shared";
                $demo = JPATH_ROOT . "/media/genesis/themes/{$theme}/template-demo";
            ...
            array_unshift($this->items['streams']['genesis-theme']['prefixes'][''], "media/genesis/themes/{$theme}/template-shared");
            array_unshift($this->items['streams']['genesis-theme']['prefixes'][''], "media/genesis/themes/{$theme}/template-demo");
            array_unshift($this->items['streams']['genesis-theme']['prefixes'][''], "media/genesis/themes/{$theme}/custom");
        }
    ...
    public function getMediaPaths(): array
    {
        ...
        $theme = isset($this->container['theme.name']) ? $this->container['theme.name'] : null;
        if ($theme && is_dir(JPATH_ROOT . "/media/genesis/themes/{$theme}/media-shared")) {
            array_unshift($paths, "media/genesis/themes/{$theme}/media-shared");
            array_unshift($paths, "media/genesis/themes/{$theme}/media-demo");
        }
    ...
    public function getEnginesPaths(): array
    {
        if (is_link(GENESIS_ROOT . '/media/genesis/engines')) {
            ...
            return ['' => ["media/genesis/engines/{$this->name}", 'media/genesis/engines/common']];
        }
        return ['' => ['media/genesis/engines']];
    }
    ...
    public function getAssetsPaths(): array
    {
        if (is_link(GENESIS_ROOT . '/media/genesis/assets')) {
            // Development environment.
            return ['' => ['genesis-theme://', "media/genesis/assets/{$this->name}", 'media/genesis/assets/common']];
        }

        return ['' => ['genesis-theme://', 'media/genesis/assets']];
    }
    ...
    public function getThemeAdminUrl(string $theme): ?string
    {
        ...
        return Route::_("index.php?option=com_genesis&view=configurations/default/layout&theme={$theme}&{$token}=1" , false);
    }
    ...
    public function displayModule($id, $attribs = [])
    {
        ...
        $isGenesis = \strpos($module->module, 'genesis') !== false;
        if (!$isGenesis && $frontEditing && $html && $user && $user->authorise('module.edit.frontend', 'com_modules.module.' . $module->id)) {
            $displayData = [
                'moduleHtml' => &$html,
                'module' => $module,
    ...
    public function displayContent($content, $params = [])
    {
        ...
        $isGenesis = \strpos($application->input->getCmd('option'), 'genesis') !== false;
    ...
    public function settings(): ?string
    {
        ...
        return Route::_('index.php?option=com_config&view=component&component=com_genesis', false) ?: '';
    }
    ...
    public function updates()
    {
        ...
        $query
            ->select('*')
            ->from('#__updates')
            ->where("element='pkg_genesis' OR extension_id IN ($extension_ids)");
