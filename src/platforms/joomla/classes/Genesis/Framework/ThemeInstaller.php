...
class ThemeInstaller extends AbstractInstaller
{
    ...
    public function render($template, $context = [])
    {
        ...
        $context += [
            ...
            'install_url' => Route::_("index.php?option=com_genesis&view=install&theme={$this->name}&{$token}=1", false),
            'edit_url' => Route::_("index.php?option=com_genesis&view=configurations/default/layout&theme={$this->name}&{$token}=1", false),
        ];
    ...
    public function addStyle($title, array $configuration = [], $home = 0)
    {
        ...
        if ($home) {
            $this->actions[] = ['action' => 'default_style_assigned', 'text' => Text::sprintf('GENESIS_INSTALLER_ACTION_DEFAULT_STYLE_ASSIGNED', $title)];
        }
    ...
    public function updateStyle($name, array $configuration, $home = null)
    {
        ...
        if ($style->id) {
            ...
            if ($home && !$style->home) {
                $this->actions[] = ['action' => 'default_style_assigned', 'text' => Text::sprintf('GENESIS_INSTALLER_ACTION_DEFAULT_STYLE_ASSIGNED', $style->title)];
            }
    ...
    public function assignHomeStyle($style)
    {
        ...
        if ($db->getAffectedRows()) {
            $this->actions[] = ['action' => 'home_style_assigned', 'text' => Text::sprintf('GENESIS_INSTALLER_ACTION_HOME_STYLE_ASSIGNED', $style->title)];
        }
    }
    ...
    public function createOutline($folder, array $params = [])
    {
        ...
        if (($this->copyCustom($folder, $target) || $created) && isset($style)) {
            ...
            if ($created) {
                $this->actions[] = ['action' => 'outline_created', 'text' => $this->translate('GENESIS_INSTALLER_ACTION_OUTLINE_CREATED', $title)];
            } else {
                $this->actions[] = ['action' => 'outline_updated', 'text' => $this->translate('GENESIS_INSTALLER_ACTION_OUTLINE_UPDATED', $title)];
            }
    ...
    public function addMenuItem(array $item, $parent_id = 1, $load = false)
    {
        ...
        $item += [
            ...
            'alias' => 'genesis',
            'note' => '',
            'link' => 'index.php?option=com_genesis&view=custom',
            'type' => 'component',
            'published' => 1,
            'parent_id' => $parent_id,
        ...
        if (!isset($this->actions["menu_{$item['menutype']}_created"])) {
            ...
            if ($update) {
                $this->actions[] = ['action' => 'menu_item_updated', 'text' => Text::sprintf('GENESIS_INSTALLER_ACTION_MENU_ITEM_UPDATED' . $postfix, $table->title, $table->path, $menu->title)];
            } else {
                $this->actions[] = ['action' => 'menu_item_created', 'text' => Text::sprintf('GENESIS_INSTALLER_ACTION_MENU_ITEM_CREATED' . $postfix, $table->title, $table->path, $menu->title)];
            }
        } elseif ($item['home']) {
            $this->actions[] = ['action' => 'menu_item_updated', 'text' => Text::sprintf('GENESIS_INSTALLER_ACTION_MENU_ITEM_HOME', $table->title, $table->path, $menu->title)];
        }
    ...
    protected function getComponent()
    {
        ...
        if (!$component_id) {
            ...
            $component_id = ComponentHelper::getComponent('com_genesis')->id;
        }
