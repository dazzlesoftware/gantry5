...
if /I "%~1"=="-n" set "GENESIS_NODE_DRY_RUN=1"

cd /d "%GENESIS_NODE_ROOT%" || (
    echo ERROR: Could not open the repository directory.
    exit /b 1
)

echo Searching for node_modules folders under:
echo %GENESIS_NODE_ROOT%
echo.

powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
    "$ErrorActionPreference = 'Stop';" ^
    "$root = [IO.Path]::GetFullPath($env:GENESIS_NODE_ROOT);" ^
    ...
    "  if ($env:GENESIS_NODE_DRY_RUN -eq '1') { Write-Host ('Would remove: ' + $path) }" ^
    "  else { Write-Host ('Removing: ' + $path); Remove-Item -LiteralPath $path -Recurse -Force -ErrorAction Stop }" ^
    "};" ^
    "if ($env:GENESIS_NODE_DRY_RUN -eq '1') { Write-Host ''; Write-Host 'Dry run complete. Nothing was removed.' }" ^
