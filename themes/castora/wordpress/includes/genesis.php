...
try {
    ...
    add_filter('template_include', static function () {
        if (is_customize_preview() && !class_exists('Timber')) {
            _e('Timber library plugin not found. ', 'genesis_castora');
        }

        _e('Theme cannot be used. For more information, please see the notice in administration.', 'genesis_castora');
