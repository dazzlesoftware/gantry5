...
PHP
        ...
        $oldFile = File::instance($locator->findResource("themes://{$parent}/genesis/theme.yaml"));
        $content = $oldFile->content();
        $oldFile->free();
        $content = preg_replace('|^( +)name: (.*)|um', '\\1name: \\2 Child', $content, 1);
        $file = File::instance($folder . '/genesis/theme.yaml');
        $file->save($content);
        $file->free();
        ...
        if ($clone) {
            $oldConfig = $locator->findResource('user://data/genesis/themes/' . $parent);
            if ($oldConfig) {
                $newConfig = $locator->findResource('user://data/genesis/themes/' . $child, true, true);
    ...
    protected function validate($type, $value)
    {
        ...
        switch ($type) {
            case 'parent':
                ...
                if (!file_exists($folder . '/genesis/theme.yaml')) {
