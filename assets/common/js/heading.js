(() => {
    ...
    const init = (scope = document) => {
        const roots = scope.matches?.("[data-heading]")
            ? [scope]
            : scope.querySelectorAll?.("[data-heading]") || [];
        roots.forEach((root) => {
            if (done.has(root)) return;
            done.add(root);
            const font = String(root.dataset.font || "").trim();
            if (!font) return;
            let family = font;
            ...
            if (family)
                root.querySelector(".g-heading").style.fontFamily =
                    `'${family.replace(/'/g, "\\'")}'`;
        });
    };
