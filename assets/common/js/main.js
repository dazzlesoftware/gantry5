(() => {
  ...
  var Menu = class {
    constructor(options = {}) {
      ...
      const menuCandidates = Array.from(document.querySelectorAll(this.selectors.mainContainer)).filter((menu) => !menu.closest("#g-offcanvas") && menu.querySelector(this.selectors.topLevel));
      this.mainContainer = menuCandidates.find((menu) => menu.matches(this.selectors.mobileTarget)) || menuCandidates[0] || null;
      if (!this.mainContainer) return;
      if (!this.mobileContainer) {
        const offcanvasBody = document.querySelector("#g-offcanvas .offcanvas-body");
        if (!offcanvasBody) return;
        const configuredBreakpoint = (_a = offcanvasBody.closest("#g-offcanvas")) == null ? void 0 : _a.getAttribute("data-g-menu-breakpoint");
      ...
      const breakpoint = this.mobileContainer.getAttribute("data-g-menu-breakpoint") || ((_b = document.querySelector("#g-offcanvas")) == null ? void 0 : _b.getAttribute("data-g-menu-breakpoint"));
  ...
  var Offcanvas2 = class {
    ...
    _checkTogglers() {
      ...
      setTimeout(() => {
        ...
        const hasOtherContent = Boolean(body && Array.from(body.children).some(
          (child) => child !== mobileContainer && !child.classList.contains("hidden") && (child.textContent.trim().length || child.children.length)
        ));
