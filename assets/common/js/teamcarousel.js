(() => {
    ...
    const applyFont = (root, selector, value) => {
        ...
        if (family)
            root.querySelectorAll(selector).forEach((node) => {
                node.style.fontFamily = `'${family.replace(/'/g, "\\'")}'`;
            });
    };
    class TeamCarousel {
        constructor(root) {
            this.root = root;
            this.track = root.querySelector(".g-teamcarousel-track");
            this.items = [...root.querySelectorAll(".g-teamcarousel-item")];
            this.index = 0;
            this.timer = null;
            this.startX = null;
            ...
            applyFont(root, ".g-teamcarousel-name", root.dataset.nameFont);
            applyFont(
                root,
                ".g-teamcarousel-designation",
                root.dataset.designationFont,
            );
        ...
        render(immediate = false) {
            const visible = this.visible(),
                pages = this.pages();
            if (this.root.dataset.loop === "enable")
                this.index = ((this.index % pages) + pages) % pages;
            else this.index = Math.max(0, Math.min(this.index, pages - 1));
            this.root.style.setProperty("--gtc-visible", visible);
        ...
        bullets(pages) {
            const holder = this.root.querySelector("[data-team-bullets]");
            if (!holder) return;
        ...
        start(reset = false) {
            ...
            this.timer = setInterval(
                () => this.move(1),
                Math.max(1000, +(this.root.dataset.interval || 4500)),
            );
        }
        stop() {
    ...
    const init = (scope = document) => {
        const roots = scope.matches?.("[data-team-carousel]")
            ? [scope]
            : scope.querySelectorAll?.("[data-team-carousel]") || [];
        roots.forEach((root) => {
            if (!instances.has(root))
                instances.set(root, new TeamCarousel(root));
        });
    };
