(() => {
    ...
    const init = (root) => {
        ...
        let index = 0;
        if (root.dataset.button1Present === "1")
            styleButtonText(
                root,
                buttons[index++],
                root.dataset.button1Font,
                root.dataset.button1Size,
                root.dataset.button1Weight,
            );
        if (root.dataset.button2Present === "1")
            styleButtonText(
                root,
                buttons[index],
                root.dataset.button2Font,
                root.dataset.button2Size,
                root.dataset.button2Weight,
            );
    };
    const scan = (scope = document) => {
        const roots = scope.matches?.("[data-feature]")
            ? [scope]
