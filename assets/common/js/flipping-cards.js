(function () {
    ...
    document.querySelectorAll('[data-flippingcards-id]').forEach(function (row) {
        let eventName = row.dataset.flippingcardsTrigger === 'hover' ? 'mouseenter' : 'click';
        let axis = row.dataset.flippingcardsAxis === 'x' ? 'X' : 'Y';
        let speed = Number(row.dataset.flippingcardsSpeed) || 500;
        row.querySelectorAll('.g-flippingcard').forEach(function (card) {
            ...
            card.addEventListener(eventName, function () {
                let flipped = card.classList.toggle('g-flippingcard-flipped');
